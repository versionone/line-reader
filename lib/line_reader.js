...
function createLineReader(readStream, options, creationCb) {
  ...
  function nextLine(cb) {
    ...
    function getLine(err) {
      ...
      var ret = bufferStr.substring(0, separatorIndex + separatorLen);
...
function open(filenameOrStream, options, cb) {
  ...
  if (typeof filenameOrStream.read == 'function') {
    ...
    readStream = fs.createReadStream(filenameOrStream, { start: options.start });
  } else {
...
function eachLine(filename, options, iteratee, cb) {
  ...
  if (!options) options = {};
  var asyncIteratee = iteratee.length === 2;

  var theReader;
  var getReaderCb;
  var prevOffset = options.start || 0;
  ...
  open(filename, options, function(err, reader) {
    ...
    function readNext() {
      reader.nextLine(function(err, line) {
        ...
        var eventArgs = {
            line: line,
            offset: prevOffset += line.length,
            last: !reader.hasNextLine(),
        }
        
        if (asyncIteratee) {
          iteratee(eventArgs, continueCb);
        } else {
          if (iteratee(eventArgs) !== false) {
            newRead();
          } else {
            finish();
          }
